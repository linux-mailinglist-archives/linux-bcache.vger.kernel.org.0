...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 22FA7584455
	for <lists+linux-bcache@lfdr.de>; Thu, 28 Jul 2022 18:45:11 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229499AbiG1QpJ (ORCPT <rfc822;lists+linux-bcache@lfdr.de>);
        Thu, 28 Jul 2022 12:45:09 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:57472 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229536AbiG1QpI (ORCPT
        <rfc822;linux-bcache@vger.kernel.org>);
        Thu, 28 Jul 2022 12:45:08 -0400
Received: from schatzi.steelbluetech.co.uk (james.steelbluetech.co.uk [92.63.139.228])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id D0611474C8
        for <linux-bcache@vger.kernel.org>; Thu, 28 Jul 2022 09:45:04 -0700 (PDT)
Received: from [10.0.5.25] (tv.ehuk.net [10.0.5.25])
        by schatzi.steelbluetech.co.uk (Postfix) with ESMTP id CC212BFC35;
        Thu, 28 Jul 2022 17:45:02 +0100 (BST)
DKIM-Filter: OpenDKIM Filter v2.10.3 schatzi.steelbluetech.co.uk CC212BFC35
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=ehuk.net; s=default;
        t=1659026702; bh=sftr+X5CZek2//2Hbp5bZBJjGayiL7lCOYQeyH5//lk=;
        h=Date:Reply-To:Subject:To:Cc:References:From:In-Reply-To:From;
        b=YCtThT1ynFlGYiLWlJ+oVdaGPg2HQiESlreacyvCcP63I9EK++EA3J4QL7JSbjx9f
         cBvrvSBpg8NNfeYoX9XmmRDzMzHAr/ussnqb51x3P/Ba5G1R+fhRilDxJE+EQwlcKs
         6CseDNVHKGU3hmzQXUtzn+dh936VvkV20GqDWA7k=
Message-ID: <06d169e0-c5d4-9447-28b8-d02b21450f30@ehuk.net>
Date:   Thu, 28 Jul 2022 17:45:02 +0100
MIME-Version: 1.0
...
References: <b968c31a-aeeb-28fd-78ef-d38344d4ecc1@ehuk.net>
 ...
 <7F912DDA-C949-480A-ACC9-E96DA34EFD8C@suse.de>
From:   Eddie Chapman <eddie@ehuk.net>
In-Reply-To: <7F912DDA-C949-480A-ACC9-E96DA34EFD8C@suse.de>
...
On 28/07/2022 17:24, Coly Li wrote:
>> 2022年7月29日 00:15，Eddie Chapman 写道：
>>
>> On 28/07/2022 16:40, Coly Li wrote:
>>>> 2022年7月28日 21:14，Eddie Chapman 写道：
>>>>
>>>> On 28/07/2022 13:37, Coly Li wrote:
>>>>>> 2022年7月22日 00:18，Eddie Chapman 写道：
>>>>>>
>>>>>> Hello,
>>>>>>
...
>>
>> This is why this situation is puzzling; the bcache device has completely gone (the /sys/block/bcacheN gone). The backing device itself still exists but is a "brain dead" non-functioning md device with no members, it has no /sys/block/mdN/bcache directory. The only thing left is the cache device which refuses to stop or unregister and is still somehow "stuck" to the backing device even though bcache has "let go" of the failed backing device.
>>
>> There surely must be a bug here somewhere, because the cache device should not be still holding onto a device which bcache has already "ejected" from itself when it failed.
> 
> 
> I feel it is some refcount related issue to bcache or the code layer below bcache. Some refcount is not 0 after the failure happens, so the cache device cannot be retired.

That's interesting and very useful to know, thanks. I guess it could be 
something with the nvme driver or more general in the block layer.
...
>>
>> Yes, this has already happened when the backing device completely failed about 2 weeks ago.
>>
>> Note, I don't care about any of the data here, neither on the cache device or backing device. The problem is that bcache won't let go of the nvme cache device when it should do, and worse it is constantly reading many hundreds of Mbyte/s from the nvme device partition it is hanging onto, affecting its performance.
>>
...
>>
>> The /sys/block/mdN/bcache directory is gone so those files are gone unfortunately. The /sys/block/mdN directory is still there, the md device still exists but in a "brain dead" state.
>>
...
>>
>> I do have these files since the cache device still exists and I cannot stop it. io_error_halflife has 0 and io_error_limit has 8
>>
...
>>
>> Right. Yes, this is very good and desirable, and it has happened here already when the backing devices failed at the start.
...
>>
>> Yes I guess I'll have to do this unfortunately. I had hoped there might be a way to force bcache to release the nvme device like it should do, like it usually does in a situation like this, as rebooting this particular server is a big pain. But if not is there anything I can do to try and get some data that might give an idea what might have gone wrong here in bcache? So that it could possibly lead to a fix, because it is a very bad bug for any who might get hit by it in the future. I guess this could also be a bug in the md raid driver in which case I could report it there if that turns out to be the case.
> 
> It might be a reference counter related issue somewhere in the IO stack. Myabe it is from bcache, or maybe lower layer below bcache. The device failure testing is hard to be fully covered, the software emulated sometimes is far from the real hardware failure. Is it possible that you may trigger a crash dump file? Then I can look inside to find some clue if I am lucky, but it cannot help your situation…

That's a great suggestion, I will try and get a crash dump before it 
gets rebooted. If I manage to do it I'll let you know.
