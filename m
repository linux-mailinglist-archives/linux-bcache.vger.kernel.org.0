...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 0265F2C54E5
	for <lists+linux-bcache@lfdr.de>; Thu, 26 Nov 2020 14:09:41 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2390024AbgKZNHk (ORCPT <rfc822;lists+linux-bcache@lfdr.de>);
        Thu, 26 Nov 2020 08:07:40 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:53200 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S2390013AbgKZNHh (ORCPT
        <rfc822;linux-bcache@vger.kernel.org>);
        Thu, 26 Nov 2020 08:07:37 -0500
Received: from casper.infradead.org (casper.infradead.org [IPv6:2001:8b0:10b:1236::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 3638DC0613D4;
        Thu, 26 Nov 2020 05:07:37 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
        d=infradead.org; s=casper.20170209; h=Content-Transfer-Encoding:MIME-Version:
        References:In-Reply-To:Message-Id:Date:Subject:Cc:To:From:Sender:Reply-To:
        Content-Type:Content-ID:Content-Description;
        bh=/BYrgHLRPPRgiWcpNwhVxjIWF/zh+iMxzZ+SWBrjyLI=; b=gurJRBIlX7FXQ7rnq9sOo8EDJN
        ;
Received: from [2001:4bb8:18c:1dd6:27b8:b8a1:c13e:ceb1] (helo=localhost)
        by casper.infradead.org with esmtpsa (Exim 4.92.3 #3 (Red Hat Linux))
        id 1kiGze-00048p-3R; Thu, 26 Nov 2020 13:07:26 +0000
...
Subject: [PATCH 37/44] block: switch partition lookup to use struct block_device
Date:   Thu, 26 Nov 2020 14:04:15 +0100
Message-Id: <20201126130422.92945-38-hch@lst.de>
...
index eaa5d65ae826b8..282fde159bd125 100644
--- a/block/genhd.c
+++ b/block/genhd.c
...
@@ -174,15 +175,18 @@ struct hd_struct *__disk_get_part(struct gendisk *disk, int partno)
  */
...
+	return part->bd_part;
 ...
 /**
@@ -256,19 +260,19 @@ struct hd_struct *disk_part_iter_next(struct disk_part_iter *piter)
...
@@ -295,10 +299,10 @@ void disk_part_iter_exit(struct disk_part_iter *piter)
 }
...
+		sector < part->bd_start_sect + bdev_nr_sectors(part);
 ...
 /**
@@ -316,10 +320,10 @@ static inline int sector_in_part(struct hd_struct *part, sector_t sector)
...
+	struct block_device *part;
 	...
 	rcu_read_lock();
@@ -338,7 +342,7 @@ struct hd_struct *disk_map_sector_rcu(struct gendisk *disk, sector_t sector)
 		}
 	}
...
+	part = disk->part0;
 out_unlock:
 	rcu_read_unlock();
 	return part;
@@ -874,7 +878,7 @@ void del_gendisk(struct gendisk *disk)
...
@@ -1181,9 +1185,9 @@ ssize_t part_stat_show(struct device *dev,
...
+		inflight = part_in_flight(p->bdev);
 	...
 	return sprintf(buf,
 		"%8lu %8lu %8llu %8u "
@@ -1223,9 +1227,9 @@ ssize_t part_inflight_show(struct device *dev, struct device_attribute *attr,
 	unsigned int inflight[2];
...
@@ -1498,9 +1502,9 @@ static int diskstats_show(struct seq_file *seqf, void *v)
...
+			inflight = part_in_flight(hd->bdev);
 		...
 		seq_printf(seqf, "%4d %7d %s "
 			   "%lu %lu %lu %u "
@@ -1618,7 +1622,7 @@ struct gendisk *__alloc_disk_node(int minors, int node_id)
 		goto out_bdput;
...
index 9c7e6730fa6098..aba750825aa0d0 100644
